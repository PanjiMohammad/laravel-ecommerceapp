                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('report.newReturn') }}" method="get">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>
                                                <strong class="text-uppercase">{{ $row->invoice }}</strong>
                                            </td>
                                            <td>
                                                <label>
                                                ...
                                                <label>
                                                    <strong>Telp:</strong> {{ $row->customer_phone }}
                                                </label><br>
                                                <label>
                                                    <strong>Alamat:</strong> {{ $row->customer_address }} {{ $row->customer->district->name }} - {{  $row->customer->district->city->name}}, {{ $row->customer->district->city->province->name }}
                                                </label>
                                            </td>
                                            <td>{{ $row->return->reason }}</td>
                                            <td>{!! $row->return->status_label !!}</td>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#exportpdf').attr('href', '/seller/reports/reportreturn/' + start.format('YYYY-MM-DD') + '+' + end.format('YYYY-MM-DD'))

            $('#created_at').daterangepicker({
                startDate: start,
                endDate: end
            }, function(first, last) {
                $('#exportpdf').attr('href', '/seller/reports/reportreturn/' + first.format('YYYY-MM-DD') + '+' + last.format('YYYY-MM-DD'))
            })
        })
    </script>
@endsection()