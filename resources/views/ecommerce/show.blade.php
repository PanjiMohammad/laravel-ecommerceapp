...
@section('orderwa')
<div class="floatwa">
	<a href="https://api.whatsapp.com/send?phone=6287889165715&amp;text=Halo%20gan,%20Saya%20mau%20order {{ $product->name }}" target="_blank"><i class="fa fa-whatsapp tombolwa"></i></a>
</div>
						...
						<div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
							<div class="carousel-inner">
								<div class="carousel-item active">
									<img class="d-block w-100" src="{{ asset('/products/' . $product->image) }}" alt="{{ $product->name }}">
								</div>
							</div>
						</div>
					</div>
								...
								@if($wishlist != NULL)
									@if($product->id == $wishlist->product_id)
										<form action="{{ route('customer.deleteWishlist', $wishlist->id) }}" onsubmit="return confirm('Kamu Yakin Menghapus Produk ini dari Daftar Wishlist ?');" method="post">
											...
											@method('DELETE')
											<div class="card_area">
												<button class="gray_btn">Hapus Daftar Keinginan</button>
											</div>
										</form>
									@endif
								@else
									<form action="{{ route('customer.save_wishlist') }}" method="POST">
										...
										<input type="hidden" name="product_id" value="{{ $product->id }}" class="form-control">
										<div class="card_area">
											<button class="main_btn">Tambah ke Daftar Keinginan</button>
										</div>
									</form>
								@endif
						@endif
...
@section('js')
    <script>
        ...
        console.log(formatHarga)
        $('.harga').text(formatHarga)
